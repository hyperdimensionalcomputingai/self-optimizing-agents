...
async def run_graphrag(test_case: dict[str, Any]) -> dict[str, str | list[dict[str, str]] | None]:
    ...
    compressed_schema = await b.CompressSchema(schema_xml, question)
    compressed_schema = compressed_schema.model_dump()
    compressed_schema_xml = db_manager.get_schema_xml(compressed_schema)
    response = await b.RAGText2Cypher(compressed_schema_xml, question, "")
    try:
        res = conn.execute(response.cypher)
        ...
        context = None
    if context:
        answer = await b.RAGAnswerQuestion(question, response.cypher, str(context))
    else:
        answer = None
    return {
        "question": question,
